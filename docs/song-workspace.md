...
- **Add Instrumental Section** is the chord-only path for intros, interludes, solos, outros, and other passages without lyrics. Tablet and desktop use the existing inline `+ Add` menu; mobile reaches the same editor through the Song Chart Global Add flow described below. The editor accepts an optional section name and 1–64 bars (default 4). Each bar is the existing Song Document V2 `instrumental` line model: empty lyric text plus an ordered chord collection, never a fake lyric or lyric-anchor token. Instrumental sections render as a content-width-driven horizontal grid: four bars per row in narrow/medium containers and eight in wide containers. Numbering is recalculated from presentation order after an insertion or deletion, and an empty bar uses a subtle em dash instead of lyric-oriented “Empty line” copy. Contextual **Add Bar**, **Edit Bar**, **Save Bar**, and **Delete Bar** wording; section rename/delete; stable IDs; autosave/reload; all chord views; transpose/capo/shapes; Performance Mode; and existing exports continue through the same paths.
- **Instrumental measure-strip presentation** keeps the normal editable Workspace close to a compact chord chart instead of a card dashboard. The visible strip is unboxed: it contains only ordered chord content separated by vertical barlines, without visible bar numbers, card backgrounds, or horizontal borders. Bar numbers remain in the localized accessible button name instead of consuming visual space. Each full-cell Edit Bar button keeps a 44 px minimum touch surface. The strip no longer stretches across the score: four-bar narrow/medium rows are capped at 288 px and eight-bar wide rows at 768 px. Multiple and long chord symbols wrap safely and empty bars show `—`. Read Mode, Performance Mode, and Print/PDF keep their existing mode-specific compact presentation and data logic.
...
At 720 px and below, normal Workspace mode hides the repeated inline insertion controls and exposes one compact **+ Add** action in the Song Chart header. It immediately opens a two-step native dialog: first choose Line, Section, or Instrumental Chords, then choose a compact, type-valid insertion boundary preview without interacting with the locked chart behind it. Back stays within the dialog and does not mutate the Song Document. Selecting a position transfers the existing fixed-body lock directly to the established Line, Section, or Instrumental editor; there is no unlock/relock frame. Cancel, X, and Escape restore the captured page position and return focus to the Global Add action. Tablet and desktop retain the existing inline insertion workflow, while Read Mode and Performance Mode do not expose Global Add.

Back to My Songs, Download, Read Mode, and Performance Mode use the same icon-before-text control structure on mobile, tablet, and desktop. Song Settings uses a workspace-scoped 42 px control-surface token and a shared label row, keeping selects, readonly Shape Key, Zoom, and Line Spacing centered within each responsive row without changing global form styles.

The editor exposes two compact navigation rows. Row 1 contains Original Key, Target Key, Capo, Shape Key, Chord Spelling, Zoom, and Line Spacing. Row 2 contains only Original, Balanced, Beginner, Roman, and Nashville, with Chord Change Hints and Smart Capo remaining a trailing action group in the same row. At 1280 and 1024 CSS pixels both navigation rows stay single-line; the 768 layout may group the two reading steppers into an internal second subrow without turning the controls into a form card, while the five modes remain one row.

At 720 px and below, the normal editor uses a mobile-only presentation without changing its controls or persistence. Back, Download, Read Mode, and Performance Mode form one equal four-card action grid with compact inline icons, while Performance retains the primary brown treatment and a subtle teal indicator. The keyboard-operable **Song settings** disclosure uses a compact sliders-icon summary; Original/Target and Capo/Shape remain two-column pairs, Chord Spelling spans the panel, and Zoom plus Line Spacing remain side by side. The mode grid is deterministic: **Original / Nashville** on the first row and **Balanced / Beginner / Roman** on the second; Chord Change Hints and text-only Smart Capo remain equal actions beneath it. These presentation rules do not apply at 768 px or wider and do not alter Read Mode, Performance Mode, the chart, or song data.
...
Create/Import, Global Add, Edit Line, Add Section, Add Instrumental Section, and Shape Picker reuse one shared background-lock utility. Opening a dialog captures the exact document scroll position, fixes the background with measured scrollbar compensation, and leaves the dialog's own scrolling enabled. X, Cancel, Escape, successful Create/Import, Save, Delete Line/Delete Bar, and shape selection restore focus where applicable and restore the captured page position once with instant scroll behavior. Global Add transfers ownership of that same lock during its modal-to-modal handoff instead of briefly unlocking the page.
