...
The editor exposes two independent navigation layers. The settings layer contains Original Key, Target Key, Capo, Shape Key, Chord Spelling, Zoom, and Line Spacing; the mode layer contains only Original, Balanced, Beginner, Roman, and Nashville. Desktop and tablet keep the settings panel visible and prevent the five-mode selector from wrapping early. At 720 px and below, one explicit, keyboard-operable **Song settings** disclosure replaces the panel; its stable summary strip is the only collapse target, and Zoom plus Line Spacing remain side by side inside the expanded panel. The mode choices use their own deterministic mobile grid.

A nested localized explanation distinguishes the three key concepts without changing their behavior: **Original Key** is the song's source key, **Target Key** is the concert key listeners should hear, and **Shape Key** is the chord-shape key the player uses after Capo is applied. For example, Target A with Capo 2 uses G shapes and sounds in A.
