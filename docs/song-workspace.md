...
- **Instrumental measure-strip presentation** keeps the normal editable Workspace close to a compact chord chart instead of a card dashboard. Each full-cell Edit Bar button uses a small centered numeric index, a larger ordered chord area, thin measure dividers, no card shadow, and a 58 px minimum touch surface. Four bars form each narrow/medium row and eight form each wide row; multiple and long chord symbols wrap safely and empty bars show `—`. The accessible button name retains the localized Bar wording plus its chord summary. Read Mode, Performance Mode, and Print/PDF keep their existing mode-specific compact presentation and data logic.
