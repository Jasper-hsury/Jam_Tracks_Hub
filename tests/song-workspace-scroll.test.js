...
test("Create and ChordPro use the same background lock and restore contract", () => {
    ...
    assert.match(events, /\[elements\.createDialog, elements\.globalAdd, elements\.sectionDialog, elements\.instrumentalDialog, elements\.lineDialog\][\s\S]*?addEventListener\("close"[\s\S]*?restoreDialogBackground/);
