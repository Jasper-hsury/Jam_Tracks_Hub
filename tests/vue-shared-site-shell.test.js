...
test("moves shared shell ownership off the eleven Vue HTML documents", () => {
...
test("retains early bootstrap, Umami, and animation compatibility assets per page", () => {
  ...
  ["index.html", "404.html", "privacy-policy.html", "service-waking.html", "feedback.html", "fretboard-trainer.html", "chord-progressions.html", "chord-dictionary.html"].forEach(htmlPath => {
...
test("keeps every legacy page on the legacy shell with no Vue mount", () => {
  legacyPages.forEach(htmlPath => {
    ...
    assert.doesNotMatch(html, /src\/entries\/(?:404|legal|privacy|service-waking|feedback|fretboard-trainer|chord-progressions|scale-explorer|chord-dictionary)\.js|vue-(?:404|legal|privacy|service-waking|feedback|fretboard-trainer|chord-progressions|scale-explorer|chord-dictionary)-root/i, htmlPath);
  });
});

test("keeps migrated page views on Vue locale ownership without legacy DOM translation", () => {
  ["ChordDictionaryView.vue", "ChordProgressionsView.vue", "FeedbackView.vue", "FretboardTrainerView.vue", "LegalView.vue", "NotFoundView.vue", "PrivacyView.vue", "ScaleExplorerView.vue", "ServiceWakingView.vue", "TracksView.vue"].forEach(fileName => {
