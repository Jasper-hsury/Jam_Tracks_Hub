...
test("moves shared shell ownership off the ten Vue HTML documents", () => {
...
test("keeps every legacy page on the legacy shell with no Vue mount", () => {
  legacyPages.forEach(htmlPath => {
    ...
    assert.doesNotMatch(html, /src\/entries\/(?:404|legal|privacy|service-waking|feedback|fretboard-trainer|chord-progressions|scale-explorer)\.js|vue-(?:404|legal|privacy|service-waking|feedback|fretboard-trainer|chord-progressions|scale-explorer)-root/i, htmlPath);
  });
});

test("keeps migrated page views on Vue locale ownership without legacy DOM translation", () => {
  ["ChordProgressionsView.vue", "FeedbackView.vue", "FretboardTrainerView.vue", "LegalView.vue", "NotFoundView.vue", "PrivacyView.vue", "ScaleExplorerView.vue", "ServiceWakingView.vue", "TracksView.vue"].forEach(fileName => {
...
test("keeps the Phase 4C version and dependency boundary unchanged", () => {
